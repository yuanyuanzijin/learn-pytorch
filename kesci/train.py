...
def train():
    ...
    train_data = AppData("data/train_8d_4p.json", iflabel=True)
    val_data = AppData("data/val_8d_4p.json", iflabel=True)
    train_dataloader = DataLoader(train_data, 128, shuffle=True, num_workers=4)
    val_dataloader = DataLoader(val_data, 128, shuffle=False, num_workers=2)

    criterion = t.nn.CrossEntropyLoss()
    learning_rate = 0.01
    weight_decay = 0.0001
    model = Sequence(14, 20, 5).cuda()
